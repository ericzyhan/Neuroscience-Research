...
function main()
    ...
    # MEMORY FIX: Only save the exact time steps we need for analysis
    # t=0 for the start of the plot, and the last 15 steps for convergence/regime checks
    save_times = vcat([0.0], collect(range(585.0, 600.0, length=15)))

    # Storage arrays for final statistics (These take negligible RAM)
    ...
    # instead of holding 121 heavy Plot objects in RAM.
    ...
    for (p_idx, p) in enumerate(P_VALS)
        for (q_idx, q) in enumerate(Q_VALS)
            ...
            # Temporary accumulators for the 50 trials
            ...
            for trial in 1:NUM_TRIALS
                ...
                # Copy W so the solver uses the correct matrix for this specific trial
                W_copy = copy(W)
                prob = remake(prob_base, u0=copy(x0), p=W_copy)
                
                # Solve ODE, saving ONLY the 16 requested time points
                sol = solve(prob, AutoTsit5(Rosenbrock23()), saveat=save_times)
                
                final_state = sol.u[end]
                
                # Make sure we have enough steps (failsafe)
                if length(sol.u) > 1
                    recent_traj = reduce(hcat, sol.u[2:end]) # The 15 points between t=585 and t=600
                    ...
                    # Calculate derivatives over the recent window
                ...
                # Save first trial data for the composite plot
                if trial == 1
                    sample_trajectories[(p_idx, q_idx)] = (sol.t, reduce(hcat, sol.u)')
                end
                
                # MEMORY FREED HERE: sol, recent_traj, final_state go out of scope and are garbage collected.
            end
    ...
    # We build the plots sequentially now that the heavy lifting is done
    ordered_plots = Any[]
    ...
    fig_stats_2 = heatmap(P_VALS, Q_VALS, active_std_heatmap, c=:plasma, clims=(0, vmax_count/4), title="Std Dev Active")
